...
from src.metric import ClassifyCorrectWithCache, ClassifyCorrectCell, ClassifyCorrectCellLam, DistAccuracy
...
class ImageNetEvelEngineLam(BasicEvalEngine):
    """ImageNetEvelEngine"""
    def __init__(self, net, eval_dataset, args):
        super().__init__()
        self.eval_dataset = eval_dataset
        self.dist_eval_network = ClassifyCorrectCellLam(net)
        self.args = args
        self.outputs = None
        self.model = None
...
def get_eval_engine(engine_name, net, eval_dataset, args):
    ...
    elif engine_name == "imagenet_lam":
        eval_engine = ImageNetEvelEngineLam(net, eval_dataset, args)
    else:
