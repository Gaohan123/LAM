...
class StateMonitor(Callback):
    """StateMonitor"""
    def __init__(self, data_size, directory=None, tot_batch_size=None, lrs=None,
            ...
            if output is not None:
                if isinstance(output, list):
                    ...
                    if float(output) > self.best_acc:
                        ...
                        rank = get_rank()
                        if rank == 0:
                            file_name = os.path.join(self._directory, "best.ckpt")
                            save_checkpoint(save_obj=cb_params.train_network, ckpt_file_name=file_name)
                            self.print("Saved checkpoint, accuracy:{:.6f}, current epoch [{}].".format(float(output),
                                                                                                  self.epoch_num))
