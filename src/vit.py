...
from mindspore.nn import Cell, Dense, Dropout, SequentialCell, ReLU
...
class ViT(Cell):
    ...
    def __init__(self, vit_config):
        ...
        self.body = body
        # self.head = head
        self.head_new = head # not load the pretrained weight of head cell
        self.norm = norm
    ...
    def construct(self, img):
        ...
        # return self.head(x)
        return self.head_new(x)

class ViT_lam(ViT):
    """Vision Transformer implementation with extra embedding output"""
    def construct(self, img):
        x = self.stem(img)
        bs, seq_len, _ = x.shape
...
def vit_base_patch32_lam(args):
    ...
    model = vit_config.network_lam(vit_config)
...
def get_network(backbone_name, args):
    ...
    elif backbone_name == 'vit_base_patch32_lam':
        backbone = vit_base_patch32_lam(args=args)
    else:
